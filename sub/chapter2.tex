...
\begin{example}[連結, 含有, 隣接, 到達可能]
  \cref{figure:SlitherLink}の完成盤面（下図）において, $h(4,1)$と$v(1,4)$は連結である. $c(3,4)$は$v(4,4)$のみを含む. $c(2,2)$と$c(2,3)$は隣接である. $h(3,8)$から$v(8,1)$へは辿ることができる.
\end{example}
...
\end{definition}
この線はグラフ理論の用語を用いたときの連結グラフとほとんど同義である. \cref{figure:SlitherLink}の完成盤面（下図）において, 集合$\{\,h(3,2),v(4,1),h(4,1),h(5,1)\,\}$は線である. この線も含有という関係性を定義することができる.
...
\begin{clearpagefigure}
  \includegraphics[width=0.85\linewidth,clip]{fig/NumberPlaceQuestion.eps}
...
\begin{clearpagefigure}
  \includegraphics[width=0.5\linewidth,clip]{fig/slitherlink.eps}
  \caption{スリザーリンクの完成可能盤面（上）と完成盤面（下）. $e_{i_y,j_y,y}=1$はオレンジ色の線で記述している. 分かりやすいように, $p(1,1)$, $p(1,8)$, $p(8,8)$, $p(8,1)$は緑色の太線で結んでいる. 実際の盤面は \cite{web:SlitherLink}からの引用. }
