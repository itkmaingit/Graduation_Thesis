...
\begin{definition}[格子点$p(i,j)$, 細胞$c(i,j)$, 横辺$h(i,j)$, 縦辺$v(i,j)$, 位置]\label{definition:VariableAtBoard}
  ...
  ただし, $y=p$のときは位置$\lambda(i_p,j_p,p)$は格子点$p(i_p,j_p$), $y=c$のときは位置$\lambda(i_c,j_c,c)$は細胞$c(i_c,j_c)$, $y=h$のときは位置$\lambda(i,j,h)$は横辺$h(i_h,j_h)$, $y=v$のときは位置$\lambda(i,j,v)$は縦辺$v(i_v,j_v)$を指すものとする. $(i_y,j_y)$の範囲は$y$によって指定されるそれぞれの変数によって定義されるものとする（$y=p$のときは$1\le i_p \le m$, $1\le j_p \le n$）.
...
さらに, パズルルールにまつわる概念として問題が存在する. それを数学的に定義するために既存のパズルルールの問題が満たしている条件を用いて定義する. まず, 問題とは「ソルバーに対して与えられる, 未知の状態が含まれた盤面と, それに対応する答え（未知の状態全てが既知になったもの）」である. 一般に既存のパズルルールにおいては, あるパズルルールの問題が存在したときそれが唯一解であることが条件である. そのことが具体的なパズルルールとして前提にあることが参考文献にて確認される. いくつか例として挙げると, Sun-Sun-Wu-Yang-Yin \cite{Sun2008}は数独（ナンバープレイス）にて作問アルゴリズム自体に, 唯一解を持たせることを念頭に置いて研究している. Fumiya-Ishihata \cite{Fumiya2022}はNagareruというパズルルールにおいても唯一解であることが前提として議論を進めている. これらから分かるように, ペンシルパズルのパズルルールにのっとった問題は, 唯一解であることを要求する. そのため, 本研究における問題とは, それが唯一解であることを要求することとする. ただしここで呼ぶ唯一解とは, 先の解とは意味が異なり, 問題を解いたとき盤面として答えが一つに定まるそのことを\textbf{唯一解}と呼ぶことに注意する.

これらを踏まえ, 以下で完成盤面, 完成可能盤面, 問題の定義を行う. ただし, 完成盤面は全ての位置の状態が既知であることを必要条件としている. よって以下で使用する\textit{conditions}には\textit{null}を認めたものがないことが前提にあることに注意する.
