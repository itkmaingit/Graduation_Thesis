\chapter{定義}\label{chapter:Definition}
\cref{chapter:Definition}では準備としてペンシルパズルと, そのパズルルールに関わる用語の定義を行う. ただし, ペンシルパズルとは「サイズ$m\times n$の平面グリッド上に存在する概念や状態に対し, 盤面が満たされるべき条件を提示した問題や, それらに付随する概念」とし, パズルルールとは, その問題を作成するための制約とする.
...
\cref{section:PuzzleRuleDefinition}では\cref{section:MathematicalDefinition}で定義する\textit{codomain}, \textit{conditions}, \textit{Hidden Information}（HI）, \textit{identification}という四つの概念を用いて, パズルルールの定義を行う.
\cref{section:Replace}では, \cref{section:MathematicalDefinition}において定義する盤面を変換するリプレイスという概念の定義を行う.
\cref{section:ConcreteConditions}では\cref{section:MathematicalDefinition}で定義する\textit{conditions}の具体的な構成方法についての説明を行う.

\section{ペンシルパズルに関する基礎的な概念の数学的定義}\label{section:MathematicalDefinition}
\cref{section:MathematicalDefinition}では\cref{section:IntroDefinition}で与えた用語を数学的に定義する. ここで定義するものは全て直感的な定義と違わないように行う.
まず, 平面グリッドを与えたとき, ペンシルパズルの盤面には左上から各格子点に対し座標$(i,j)$を割り振ることができる（\cref{figure:Coordinate}）. そうしたとき, グリッドにおける格子点, 細胞, 横辺, 縦辺に対して\cref{figure:VariableAtBoard}で表されるように変数を与えることができる. ただし, 格子点, 細胞, 横辺, 縦辺という言葉は\cref{figure:Board}に示したものに対応させることとする.
...
\begin{definition}[格子点$p(i,j)$, 細胞$c(i,j)$, 横辺$h(i,j)$, 縦辺$v(i,j)$, 位置]\label{definition:VariableAtBoard}
  グリッドのサイズを$m\times n$としたとき, \textbf{格子点}$p(i,j)$($(i,j)\in \mathbb{N}^2$)を
  \begin{equation}
  ...
  \end{align}
  と定義する. ただし, $h(i,j)$, $v(i,j)$の詳細に興味がない場合は, 任意の辺という意味で辺$e(i_y,j_y,y)$を$y$ $\in$ $\{\,h,v\,\}$に対して, $e(i_y,j_y,y) \in \{\,h(i_h,j_h), v(i_v,j_v)\,\}$として定義する. ただし, $y=h$のとき辺$e(i_h,j_h,h)$は横辺$h(i_h,j_h)$と定義する. $y=v$のとき辺$e(i_v,j_v,v)$は縦辺$v(i_v,j_v)$と定義する. $(i_y,j_y)$の範囲は$y$によって指定されるそれぞれの変数によって定義されるものとする（$y=h$のときは$1\le i_h \le m-1$, $1\le j_h \le n$）. また, 格子点, 細胞, 横辺, 縦辺の集合の詳細そのものに興味がない場合は任意の格子点, 細胞, 横辺, 縦辺という意味で位置$\lambda(i_y,j_y,y)$を$y$ $\in$ $\{\,p,c,h,v\,\}$に対して, $\lambda(i_y,j_y,y) \coloneqq \{\,p(i_p,j_p),c(i_c,j_c),h(i_h,j_h),v(i_v,j_v)\,\}$と定義する.
  ただし, $y=p$のとき, 位置$\lambda(i_p,j_p,p)$は格子点$p(i_p,j_p$)と定義する. $y=c$のとき, 位置$\lambda(i_c,j_c,c)$は細胞$c(i_c,j_c)$と定義する. $y=h$のとき, 位置$\lambda(i,j,h)$は横辺$h(i_h,j_h)$と定義する. $y=v$のとき, 位置$\lambda(i,j,v)$は縦辺$v(i_v,j_v)$と定義する. $(i_y,j_y)$の範囲は$y$によって指定されるそれぞれの変数によって定義されるものとする（$y=p$のときは$1\le i_p \le m$, $1\le j_p \le n$）.
  これら添え字付きの変数を改めて格子点, 細胞, 横辺, 縦辺と呼び, これらをまとめた総称として\textbf{位置}と呼ぶこととする.
\end{definition}
ペンシルパズルにおいて, 位置には状態が存在し, これをそのまま\textbf{状態}と定義する. ソルバーにある特定の位置に対し状態が明示されているそのことを\textbf{既知}と呼び, その具体的な数値や定数を\textbf{解}と呼ぶ（e.g. 3や$x_1$. ）. ただし, ここでのソルバーとは\cref{section:IntroDefinition}で触れたように, あるパズルルールの問題が存在したとき, 問題を解く解答者, あるいは問題を解くアルゴリズムのことを指すこととする. しかし特別に辺に関しては「書かれている」ときは1, 「書かれていない」ときは0に対応させるものとする （参照：\cref{definition:Identification}）. また, 解として「状態を持たない」ということを明示する場合の定数を特別に$\emptyset$と表現する. 状態が明示されていないことを\textbf{未知}と呼び, そのときの値を特別に\textbf{\textit{null}}と表現する.
...
\begin{definition}[\textit{codomain}]\label{definition:Codomain}
  位置に対応する状態がとりうる集合のことを\textbf{\textit{codomain}}と定義する. 格子点, 細胞, 横辺, 縦辺の\textit{codomain}を表す集合としてそれぞれ$\mathbb{P}$, $\mathbb{C}$, $\mathbb{H}$, $\mathbb{V}$を用いる. $\mathbb{H}$, $\mathbb{V}$の\textit{codomain}が一致している場合にはまとめて$\mathbb{E}$と記述する. \textit{codomain}の詳細に興味がなく, \textit{codomain}全体を考えるときにはそれら集合を含意するものとして$\Lambda$を使用する.
\end{definition}

ここで, 位置が既知であるとき, それぞれの位置には解として具体的な値（1や3などの数値, あるいは$x_1$などの記号）が対応するがその解は列をなしていて同一の値が含んだ列となることがある.
よって\textit{codomain}と解の列は同一視できないことに注意する. また, パズルルールによっては格子点, 細胞, 横辺, 縦辺のいずれかに全ての$(i,j)$において状態を持たない場合があり, そのときに限っては\textit{codomain}は$\emptyset$と記述し, 位置に対応する状態が存在しないことを表すこととする. ここで$\mathbb{P}=\{\,\emptyset\,\}$と$\mathbb{P}=\emptyset$は同義であるが, 簡潔さを優先し$\mathbb{P}=\emptyset$を用いる. 以下に\textit{codomain}の例を挙げる. 例としては既存のパズルルールのスリザーリンクを用いる. ただし, スリザーリンクのパズルルールは以下のものである（\cite{web:SlitherLink}）.
\begin{example}[スリザーリンクのパズルルール]\label{example:SlitherLinkRule}\textup{}
  \begin{enumerate}
    \item \jaitalic{点と点の間にタテヨコに線を引き, 全体で１つの輪っかを作りましょう.}\label{SlitherLinkRule_1}
    \item \jaitalic{４つの点で作られた正方形の中にある数字は, その正方形の辺に引く線の数を表しています.}\\
          \jaitalic{数字のない正方形には, 何本の線を引くかわかりません.}\label{SlitherLinkRule_2}
    \item \jaitalic{線を交差させたり, 枝分かれさせたりしてはいけません.}\\（文章は \cite{web:SlitherLink}から引用. ）\label{SlitherLinkRule_3}
  \end{enumerate}
\end{example}
...
\end{definition}
集合論の用語を用いると, 状態の列$(\lambda_{1,1,y_{(1,1)}}, \lambda_{1,2,y_{(1,2)}},\ldots)$と写像\\
...
ここで$\{\,\lambda_{i_y,j_y,y}\,\}$は$\{\,\lambda_{1,1,y_{(1,1)}}, \lambda_{1,2,y_{(1,2)}},\ldots\,\}$である. 写像$\bm{\lambda}$は$\{\,\lambda(i_y,j_y,y)\,\}$によって添え字付けられた族である. またこのとき, $\{\,\lambda(i_y,j_y,y)\,\}$は添字集合で, 位置$\lambda(i_y,j_y,y)$はこの写像の添字である.

状態が既知である位置とその解との対応は\cref{definition:Mapping}のように添字によってラベル付けされる. また, 解同士の関係は\cref{definition:Codomain}直後で記述したように実際に取る値の如何に関わらず別物として扱う.
位置と, その解との対応（\cref{definition:Mapping}）が全て分かっているとき, 写像はそれらの取る値を全て列として記述すれば$\bm{\lambda}\Leftrightarrow \{\,\lambda_{i_y,j_y,y}\,\}$と同一視することができる.

このような種々の定義を導入することにより, グリッドのサイズ$m$, $n$と\textit{codomain}として$\mathbb{P}$, $\mathbb{C}$, $\mathbb{E}$を与えたとき任意の盤面を位置とその状態の対応によって定義することができる.
...
このとき, $B$は集合として$B=\{\,\lambda(i_y,j_y,y)\,\}$を考え, 状態$\lambda_{i_y,j_y,y}$も$\bm{\lambda}$により$B$の元として扱うこととする.
ただし前と同様$\{\,\lambda(i_y,j_y,y)\,\}$とは$\{\,\lambda(1,1,y_{(1,1)}), \lambda(1,2,y_{(1,2)}),\ldots\,\}$を指すものとする.

さらに, パズルルールにまつわる概念として問題が存在する. それを数学的に定義するために既存のパズルルールの問題が満たしている条件を用いて定義する. まず, 問題とは「ソルバーに対して与えられる, 未知の状態が含まれた盤面と, それに対応する答え（未知の状態全てが既知になったもの）」である. 一般に既存のパズルルールにおいては, あるパズルルールの問題が存在したとき, それは唯一解であることが条件である. そのことが具体的なパズルルールとして前提にあることが参考文献にて確認される. いくつか例として挙げると, Sun-Sun-Wu-Yang-Yin \cite{Sun2008}は数独（ナンバープレイス）にて作問アルゴリズム自体に, 唯一解を持たせることを念頭に置いて研究している. Fumiya-Ishihata \cite{Fumiya2022}はNagareruというパズルルールにおいて唯一解であることが前提として議論を進めている. これらから分かるように, ペンシルパズルのパズルルールに則った問題は, 唯一解であることを要求する. そのため, 本研究における問題とは, それが唯一解であることを要求することとする. ただしここで呼ぶ唯一解とは, 先の解とは意味が異なり, 問題を解いたとき盤面として答えが一つに定まるそのことを\textbf{唯一解}と呼ぶことに注意する.

これらを踏まえ, 以下で完成盤面, 完成可能盤面, 問題の定義を行う. ただし, 完成盤面は全ての位置の状態が既知であることを必要条件としている. よって以下で使用する\textit{conditions}は\textit{null}を含めないことが前提であることに注意する.
...
\begin{definition}[完成可能盤面]
  ある盤面が存在して, その盤面において全ての位置の状態が既知としたとき, その解が全て\textit{conditions}に従う完成盤面がただ一つしかないような盤面を\textbf{完成可能盤面}と定義する.
\end{definition}
...
問題の集合が要素として2以上であるから, 必ず非完成盤面も問題に含まれることに注意する. 以上のような定義を行うことで, 問題は唯一解であることを保証する. さらに, ある問題に含まれる完成可能盤面を, その完成可能盤面に対応する完成盤面へと変換するその操作のことを問題を\textbf{解く}と呼ぶこととする.
...
\end{equation}
このように与えることであるパズルルールが存在したとき, 完成盤面は$X$の元であると言える. そのため, このように記述することで\textit{conditions}は完成盤面$X$と同値とみなすことができる.
...
なぜなら完成盤面から解を隠した盤面があったとき, その盤面における全ての位置の状態を既知にしたとき全ての解が\textit{conditions}に従う完成盤面は一つとは限らないためである（元の完成盤面以外に違う完成盤面が\textit{conditions}に従う可能性がある. ）. よってそれはあくまで盤面であり完成可能盤面とは限らないことに注意する.
\begin{example}\label{example:HI}
  例として\cref{figure:HIExample_Complete}のスリザーリンクの完成盤面からHIを
  \begin{align}
     & \{\,c(2,1)\,\}        \\
     & \{\,e(i_y,j_y,y)\,\}.
  \end{align}
  を選んだものが\cref{figure:HIExample_Incomplete}であるが, これは完成盤面として他に\cref{figure:HIExample_OtherComplete}が考えられる. よって\cref{figure:HIExample_Incomplete}は非完成盤面ではない.
\end{example}
以上の\cref{example:HI}より, 完成盤面の存在は問題の存在と同値にならない. しかし, 既存のパズルルールにおいては藤原 \cite{Fujiwara2022}より以下（\cref{theorem:AutoGeneration}）が成立することが示されている. ただし既存のパズルルールとは, 藤原 \cite{Fujiwara2022}が想定しているパズルルールの適用範囲内に従う.
\begin{theorem}\label{theorem:AutoGeneration}
  既存のパズルルールにおいて, ある完成盤面が存在したとき, その完成盤面から非完成盤面を任意に生成することができる. つまり, 完成盤面の存在は問題の存在と同値である.
\end{theorem}
\cref{theorem:AutoGeneration}における具体的な生成方法については, \cref{section:GenericAlgorithm}で触れる. ただし, 本研究で定義する全てのパズルルールの任意の完成盤面に, 未知の状態を含む完成可能盤面が存在しているわけではないことに注意する. 完成盤面の存在が問題の存在と同値にする\textit{conditions}の制約条件は解明できていない. その制約条件が解明されればいかなるパズルルールにおいても\cref{theorem:AutoGeneration}から問題を作成することができるため, これを今後の研究課題とする.
...
\cref{section:MathematicalDefinition}の概念を用いることにより, 以下のようにペンシルパズルのパズルルールを定義する.
...
\cref{section:ConcreteConditions}ではパズルルールの特性を左右する\textit{conditions}を具体的に構成する方法について\cref{example:SlitherLinkConditions}や\cref{chapter:Demonstration}で登場するもののみについて説明を行う. ただし, ここで触れる構成方法については限定的なものであり, 他のパズルルールの\textit{conditions}を記述する際には新たな概念を導入する必要がある. ただし, それは本論文の主旨とは少し異なるためそれらについては触れないものとする.
...
既存のパズルルールにおいては, ある位置の状態は他の位置の状態によって決まるものがほとんどである. そのため, 位置の関係性を数学的に定義しておく必要がある. そのため, \cref{subsection:RelationDefinition}では\cref{section:MathematicalDefinition}で定義した位置同士の関係性について定義を行う. ただし, ここでは完成盤面を仮定する.
...
\begin{definition}[連結]\label{definition:Connection}
  辺$e(i_y,j_y,y)$と辺$e(i_{y'}',j_{y'}',y')$が\textbf{連結}であることを, 以下が成立することで定義する
  \begin{equation}
    \begin{cases}
...
\begin{definition}[含有]\label{definition:Contain}
  辺$e(i_y,j_y,y)$が格子点$p(i',j')$を\textbf{含む（含有する）}ことを, 以下が成立することで定義する
  \begin{equation}
    \begin{cases}
      {e_{i_y,j_y,y}=1               } \\
    ...
    \end{cases}
  \end{equation}
  以下同様に, 細胞$c(i,j)$が辺$e(i_{y'}',j_{y'}',y')$を含むことと, 細胞$c(i,j)$が格子点$p(i',j')$を含むことを以下が成立することでそれぞれ定義する
  \begin{equation}
    \begin{cases}
      {e_{i_{y'}',j_{y'}',y'}=1 } \\
  ...
  \end{align}
\end{definition}
このような含有の概念を考えることで細胞, 辺が持つ集合を考えることができる. それらを$P_{c(i,j)}$, $E_{c(i,j)}$, $P_{e(i_y,j_y,y)}$として以下で定義する. ただし, $c(i,j)\supset e(i_{y'}',j_{y'}',y')$などと記述する場合には$c(i,j)$に$e(i_y,j_y,y)$が（本研究における）含まれるという意味で用いる.
...
\begin{example}[連結, 含有, 隣接, 到達可能]
  \cref{figure:SlitherLink}の完成盤面において, $h(4,1)$と$v(1,4)$は連結である. $c(3,4)$は$v(4,4)$のみを含む. $c(2,2)$と$c(2,3)$は隣接である. $h(3,8)$から$v(8,1)$へは辿ることができる.
\end{example}
...
\end{definition}
この線はグラフ理論の用語を用いたときの連結グラフとほとんど同義である. \cref{figure:SlitherLink}の完成盤面において, 集合$\{\,h(3,2),v(4,1),h(4,1),h(5,1)\,\}$は線である. この線も含有という関係性を定義することができる.
\begin{definition}[線の含有]
  線$L$が線$L'$を含むとは以下が成立することで定義する
  \begin{equation}
    L\supset L'.
  \end{equation}
  同様に線$L$が辺$e(i_y,j_y,y)$, 格子点$p(i,j)$が含むことを以下の成立によりそれぞれ定義する
  \begin{equation}
    \begin{cases}
...
\begin{example}[スリザーリンクの\textit{conditions}]\label{example:SlitherLinkConditions}
...
\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/Board.eps}
  \caption{盤面と, 位置の名称の対応}
  \label{figure:Board}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.85\linewidth,clip]{fig/NumberPlace.eps}
  \caption{数独（ナンバープレイス）の完成盤面. 実際の盤面は \cite{web:Sudoku}からの引用. }
  \label{figure:NumberPlace}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.85\linewidth,clip]{fig/NumberPlaceIncomplete.eps}
  \caption{数独（ナンバープレイス）の完成可能盤面. 実際の盤面は \cite{web:Sudoku}からの引用. }
  \label{figure:NumberPlaceQuestion}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.8\linewidth,clip]{fig/Coordinate.eps}
  \caption{グリッドに割り振る座標}
  \label{figure:Coordinate}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/VariableAtBoard.eps}
  \caption{位置と変数の対応}
  \label{figure:VariableAtBoard}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/Slitherlink.eps}
  \caption{スリザーリンクの完成盤面. 実際の盤面は \cite{web:SlitherLink}からの引用. }
  \label{figure:SlitherLink}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/SlitherLinkIncomplete.eps}
  \caption{\cref{figure:SlitherLink}に対応するスリザーリンクの完成可能盤面. 実際の盤面は \cite{web:SlitherLink}からの引用. }
  \label{figure:SlitherLinkIncomplete}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/SlitherLinkOtherIncomplete.eps}
  \caption{\cref{figure:SlitherLink}に対応する\cref{figure:SlitherLinkIncomplete}とは異なるスリザーリンクの完成可能盤面. 実際の盤面は \cite{web:SlitherLink}からの引用. }
  \label{figure:SlitherLinkOtherIncomplete}
\end{clearpagefigure}
...
\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/HIExample_Complete.eps}
  \caption{スリザーリンクの完成盤面}
  \label{figure:HIExample_Complete}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/HIExample_Incomplete.eps}
  \caption{\cref{figure:HIExample_Complete}から$c(2,1)$と$\{\,e(i_y,j_y,y)\,\}$を隠した盤面}
  \label{figure:HIExample_Incomplete}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/HIExample_OtherComplete.eps}
  \caption{\cref{figure:HIExample_Incomplete}に対応する\cref{figure:HIExample_OtherComplete}と異なる完成盤面}
  \label{figure:HIExample_OtherComplete}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/Cross.eps}
  \caption{cross関数}
  \label{figure:cross}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=\linewidth,clip]{fig/Cycle.eps}
  \caption{cycle関数}
  \label{figure:cycle}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.5\linewidth,clip]{fig/Replace.eps}
  \caption{上がリプレイス前の盤面, 下がリプレイス後の盤面. 細胞と格子点が入れ替わっていることが分かる. }
