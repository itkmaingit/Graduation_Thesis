\chapter{定義}\label{chapter:Prepare}
...
まず, 平面グリッドを与えたとき, ペンシルパズルの盤面には左上から各格子点に対し座標$(i,j)$を割り振ることができる（\cref{figure:Coordinate}）. そうしたとき, グリッドにおける格子点, 細胞, 横辺, 縦辺に対して\cref{figure:VariableAtBoard}で表されるように変数を与えることができる.
...
\begin{definition}[格子点$p(i,j)$, 細胞$c(i,j)$, 横辺$h(i,j)$, 縦辺$v(i,j)$, 位置]\label{definition:VariableAtBoard}
  ...
  \end{align}
  と定義する. ただし, $h(i,j)$, $v(i,j)$の詳細に興味がない場合は, 任意の辺という意味で辺$e(i_y,j_y,y)$を$y$ $\in$ $\{\,h,v\,\}$に対して, $e(i_y,j_y,y) \in \{\,h(i_h,j_h), v(i_v,j_v)\,\}$として定義する. ただし, $y=h$のとき辺$e(i_h,j_h,h)$は横辺$h(i_h,j_h)$, $y=v$のとき辺$e(i_v,j_v,v)$は縦辺$v(i_v,j_v)$を指すものとする. $(i_y,j_y)$の範囲は$y$によって指定されるそれぞれの変数によって定義されるものとする（$y=h$のときは$1\le i_h \le m-1$, $1\le j_h \le n$）.

  また, 格子点, 細胞, 辺の集合の詳細そのものに興味がない場合は任意の格子点, 細胞, 辺という意味で位置$\lambda(i_y,j_y,y)$を$y$ $\in$ $\{\,p,c,h,v\,\}$に対して, $\lambda(i_y,j_y,y) \coloneqq \{\,p(i_p,j_p),c(i_c,j_c),h(i_h,j_h),v(i_v,j_v)\,\}$と定義する.
...
\begin{example}
  例として\cref{figure:HIExample1}にある, スリザーリンクの完成盤面からHIを
  \begin{align}
     & \{\,c(2,1)\,\}        \\
     & \{\,e(i_y,j_y,y)\,\}.
  \end{align}
  を選んだものが\cref{figure:HIExample2}であるが, これは完成盤面として他に\cref{figure:HIExample3}が考えられる. よって\cref{figure:HIExample2}は非完成盤面ではない.
\end{example}
...
\begin{theorem}\label{theorem:AutoGeneration}
  ある完成盤面が存在したとき, その完成盤面から非完成盤面を任意に生成することができる. つまり, 完成盤面の存在は問題の存在と同値である.
\end{theorem}
...
\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/Coordinate.eps}
  \caption{グリッドに割り振る座標}
...
\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/HIExample1.eps}
  \caption{スリザーリンクの完成盤面}
  \label{figure:HIExample1}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/HIExample2.eps}
  \caption{\cref{figure:HIExample1}から$c(2,1)$を隠した盤面}
  \label{figure:HIExample2}
\end{clearpagefigure}

\begin{clearpagefigure}
  \includegraphics[width=0.7\linewidth,clip]{fig/HIExample3.eps}
  \caption{\cref{figure:HIExample2}に対応する\cref{figure:HIExample3}と異なる完成盤面}
  \label{figure:HIExample3}
\end{clearpagefigure}
